...
*/
class UsersProfileCardWidgetAdminPageFramework_Form_View___Script_RepeatableField extends UsersProfileCardWidgetAdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateUsersProfileCardWidgetAdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        if ( ! $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions ) {
            $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions = [];
        }
        if ( ! $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( sFieldsContainerID ) ) {     
            $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ];
        ...
        $( nodeThis ).find( '.users-profile-card-widget-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.users-profile-card-widget-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.users-profile-card-widget-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.users-profile-card-widget-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
        ...
        $( nodeThis ).find( '.repeatable-field-add-button' ).click( function() {
            $( this ).addUsersProfileCardWidgetAdminPageFrameworkRepeatableField();
        ...
        $( nodeThis ).find( '.repeatable-field-remove-button' ).click( function() {
            $( this ).removeUsersProfileCardWidgetAdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });     
        
        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.users-profile-card-widget-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + sFieldsContainerID ).find( '.users-profile-card-widget-field' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addUsersProfileCardWidgetAdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addUsersProfileCardWidgetAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.users-profile-card-widget-field' ).attr( 'id' );    
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.users-profile-card-widget-fields' );
        var sFieldsContainerID  = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.users-profile-card-widget-repeatable-field-buttons' );
            $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ] = {    
        ...
        var _iFadein  = $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadein' ];
        var _iFadeout = $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ];

        // Show a warning message if the user tries to add more fields than the number of allowed fields.
        var sMaxNumberOfFields = $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ]['max'];
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.users-profile-card-widget-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.users-profile-card-widget-repeatable-field-buttons' ).last();
         ...
         * */
        nodeNewField.updateUsersProfileCardWidgetAdminPageFrameworkRepeatableFields();
        ...
        // For nested fields,
        $( nodeNewField ).find( '.users-profile-card-widget-field' ).each( function( iIterationIndex ) {    
    ...
    $.fn.removeUsersProfileCardWidgetAdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.users-profile-card-widget-field' );
        var nodeFieldsContainer = $( this ).closest( '.users-profile-card-widget-fields' );
        var sFieldsContainerID  = nodeFieldsContainer.attr( 'id' );
        
        /* If the set minimum number of fields already exists, do not remove */
        var sMinNumberOfFields  = $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'min' ];
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.users-profile-card-widget-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.users-profile-card-widget-repeatable-field-buttons' ).last();
            ...
            var _iFadeout = $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ]
                ? $.fn.aUsersProfileCardWidgetAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ]
                : 500;
            nodeMessage.delay( 2000 ).fadeOut( _iFadeout );
            return;     
        }     
