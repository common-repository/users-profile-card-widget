...
*/
class UsersProfileCardWidgetAdminPageFramework_Form_View___SectionCaption extends UsersProfileCardWidgetAdminPageFramework_FrameworkUtility {
    ...
    private function _getCaption(array $aSectionset, $iSectionIndex, $aFieldsets, $aFieldErrors, $aFieldTypeDefinitions, $aCallbacks, $oMsg) {
        if (!$aSectionset['description'] && !$aSectionset['title']) {
            return "<caption class='users-profile-card-widget-section-caption' style='display:none;'></caption>";
        }
    ...
    private function _getSectionError($aSectionset, $aFieldErrors) {
        ...
        return $_sSectionError ? "<div class='users-profile-card-widget-error'><span class='section-error'>* " . $_sSectionError . "</span></div>" : '';
    }
    private function _getCaptionTitle($aSectionset, $iSectionIndex, $aFieldsets, $aFieldTypeDefinitions) {
        $_oSectionTitle = new UsersProfileCardWidgetAdminPageFramework_Form_View___SectionTitle(array('title' => $aSectionset['title'], 'tag' => 'h3', 'section_index' => $iSectionIndex, 'sectionset' => $aSectionset,), $aFieldsets, $this->aSavedData, $this->aFieldErrors, $aFieldTypeDefinitions, $this->oMsg, $this->aCallbacks);
        return "<div " . $this->getAttributes(array('class' => 'users-profile-card-widget-section-title', 'style' => $this->getAOrB($this->_shouldShowCaptionTitle($aSectionset, $iSectionIndex), '', 'display: none;'),)) . ">" . $_oSectionTitle->get() . "</div>";
    }
    private function _getCaptionDescription($aSectionset, $hfSectionCallback) {
        ...
        $_oSectionDescription = new UsersProfileCardWidgetAdminPageFramework_Form_View___Description($aSectionset['description'], 'users-profile-card-widget-section-description');
        return "<div class='users-profile-card-widget-section-description'>" . call_user_func_array($hfSectionCallback, array($_oSectionDescription->get(), $aSectionset)) . "</div>";
    }
