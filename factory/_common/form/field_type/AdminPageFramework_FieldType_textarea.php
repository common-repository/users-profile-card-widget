...
*/
class UsersProfileCardWidgetAdminPageFramework_FieldType_textarea extends UsersProfileCardWidgetAdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    ...
    jQuery().registerUsersProfileCardWidgetAdminPageFrameworkCallbacks( {
        ...
        added_repeatable_field: function( oCloned, sFieldType, sFieldTagID, iCallType, iSectionIndex, iFieldIndex ) {
            ...
            var _oSettings = jQuery().getUsersProfileCardWidgetAdminPageFrameworkInputOptions( _oWrap.attr( 'data-id' ) );  

            // Elements
            var _oField              = oCloned.closest( '.users-profile-card-widget-field' );
        ...
        stopped_sorting_fields : function( oSortedFields, sFieldType, sFieldsTagID, iCallType ) { 
            ...
            // Update the editor.
            oSortedFields.children( '.users-profile-card-widget-field' ).each( function( iIndex ) {
                ...
                var oSettings = jQuery().getUsersProfileCardWidgetAdminPageFrameworkInputOptions( oWrap.attr( 'data-id' ) );   
        ...
        saved_widget : function( oWidget ) { 
            ...
            jQuery( oWidget ).find( '.users-profile-card-widget-field' ).each( function( iIndex ) {
                ...
                // Store the settings.
                jQuery().storeUsersProfileCardWidgetAdminPageFrameworkInputOptions( 
                    oWrap.attr( 'data-id' ), 
                    { 
...
JAVASCRIPTS;
    ...
    protected function getStyles() {
        return ".users-profile-card-widget-field-textarea .users-profile-card-widget-input-label-string {vertical-align: top;margin-top: 2px;} .users-profile-card-widget-field-textarea .wp-core-ui.wp-editor-wrap {margin-bottom: 0.5em;}.users-profile-card-widget-field-textarea.users-profile-card-widget-field .users-profile-card-widget-input-label-container {vertical-align: top; float: left;clear: both;} .postbox .users-profile-card-widget-field-textarea .users-profile-card-widget-input-label-container {width: 100%;}.users-profile-card-widget-field-textarea textarea {max-width: 100%;}";
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'users-profile-card-widget-field-textarea-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='users-profile-card-widget-input-label-container {$_sClassSelector}'>", "<label for='" . $aField['attributes']['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span class='users-profile-card-widget-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $_sLabel . "</span>" : '', $this->_getEditor($aField), $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='users-profile-card-widget-textarea-enabler'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}
